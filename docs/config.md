...
| `tile_cache.dir`/`tile_cache.max_mb` | Where the `html/` CGI browser keeps its on-disk cache of 3D Galaxy Map tiles (see `../src/html/lib/tilecache.py`), and roughly how big that cache may grow before its oldest files are pruned. An empty `dir` (the default) means `/var/cache/planetgen/tiles` when Apache can create it, else a `planetgen-tiles` folder in the system temp directory. `max_mb` of `0` turns the disk cache off. Equivalent to `PLANETGEN_TILE_CACHE_DIR`/`PLANETGEN_TILE_CACHE_MAX_MB`. |
