...
| `../src/html/lib/tilecache.py` | The web layer's on-disk cache of 3D Galaxy Map tiles: `fetch_tiles` serves each requested tile from `<cache dir>/<db>/<stamp>/` when present and asks `GET /api/galaxy/tiles` only for the rest. The stamp (`GET /api/galaxy/stamp`) is remembered for 60 s, and a new one deletes the old stamp's tiles. Size-capped (`tile_cache.max_mb`, oldest files pruned first); location from `PLANETGEN_TILE_CACHE_DIR`/`tile_cache.dir` (see `config.md`). Fails open: an unwritable directory or bad file just means an API call. Not web-accessible. |
