...
| `../src/html/lib/galaxymap3d.py` | Builds `galaxy3d.py`'s "Galaxy Map (3D)" panel: the canvas/controls/info-panel markup, plus the one starting JSON payload (zoom-range numbers, and `queryDb.galaxy_view`'s own placed/planned/density lists for the zoomed-all-the-way-out view) `static/galaxymap3d.js` reads on first paint -- every later payload, as the camera moves, is fetched by that script directly and never passes through this module. `view_radius_bounds` (the zoom floor/ceiling: a couple of sector-widths up to this galaxy's own real outer edge) is a pure function `galaxy3d.py` also calls directly, before this module's own panel-rendering function runs. Not web-accessible. |
