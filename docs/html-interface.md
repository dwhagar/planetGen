...
| `../src/html/static/navform.js` | Loaded on every page (`lib/page.py`'s `render`). Lets a `data-nav-target`/`data-nav-params` element (a Galaxy Map/Sector Map/NAV Map marker -- see `lib/fmt.py`'s `data_nav_params`) navigate by posting a throwaway hidden form, since a `<form>` can't nest inside the SVG shape those markers are drawn as -- everywhere else in `html/`, `lib/fmt.py`'s `post_link` builds a real, no-JS-required `<form>` instead. Served directly, same as `style.css`. |
