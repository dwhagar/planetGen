...
actually placed in the galaxy (small dots for every galaxy-placed nebula/
asteroid field), and a true-position "System Map" per system, plotting
...
| `../src/html/lib/starmap.py` | Builds `sector.py`'s "Sector Map" panel: a real CSS 3D scene (`perspective`-free/orthographic, `transform-style: preserve-3d`) of the sector's cube -- a 6-bordered-`<div>`-face wireframe plus one billboarded `<div>` per star, positioned via plain layout (`left`/`top`) for x/y and `transform: translateZ()` for z, plus one billboarded translucent circle per nearby nebula/asteroid field (`_cloud_html`), sized by `radius_ly` and positioned directly in the galaxy frame (no rotation needed, unlike a star system's sector-local position -- see `queryDb.phenomena_near_sector`). Star dot radius comes from `radius_km` (square-root scaled against the Sun); dot color from `star_type`'s spectral letter (`SPECTRAL_CLASS_COLORS`) shaded by `luminosity_w` and nudged by where `temperature_k` falls in that spectral class's range -- not a raw Kelvin-to-RGB blackbody map, so "White Giant" reads white and "Blue Giant" reads blue regardless of temperature. Not web-accessible. |
