...
| File | Purpose |
|---|---|
| [`planetgen.conf.example`](../examples/apache/planetgen.conf.example) | Example Apache2 virtual host: `DocumentRoot` at `/var/lib/planetGen/html`, CGI handling for `../src/html/*.py`, access rules (denying direct requests to `../src/html/lib/`), and a `WSGIScriptAlias /api` mounting the Flask API (`../src/api/`, via `../src/wsgi.py`) on the same vhost -- see [`api.md`](api.md#deploying-behind-apache-mod_wsgi) and that example file's own header comment for why the API is aliased in rather than moved into `DocumentRoot`. Copy to `/etc/apache2/sites-available/`, edit, and enable with `a2ensite` -- the one step `install.sh` deliberately leaves manual. |
