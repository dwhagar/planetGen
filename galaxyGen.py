...
Three generation modes, all but the last reducing to one call of
...
- **Random-start mode** (neither `--shell` nor `--center-sector` given --
  `run_random_start`): the zero-argument default. Picks a uniformly
  random (by volume), not-yet-occupied sector address somewhere within a
  real Milky-Way-scale galaxy (`--max-shell`, defaulting to
  `program_constants.GALAXY_RADIUS_PC`'s own nearest shell), generates it,
  then falls straight through to local-neighborhood mode's own logic to
  generate every sector within `--radius-pc` of it too (defaulting to
  `program_constants.RANDOM_START_NEIGHBORHOOD_RADIUS_LY`, 100 ly, in
  every direction) -- so `galaxyGen.py` with no arguments at all creates a
  whole small starmap around a fresh, randomly chosen starting point in
  one run.

Every mode skips any candidate slot a sector already occupies
(`stellarObjects._db.get_occupied_shell_slots`/`get_sector_id_at`) rather
than regenerating or erroring on it, so re-running this script over a
partially-generated shell or neighborhood only fills in what's missing.
...
def process_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Generates many sectors as one galaxy -- see this script's module docstring for the three "
               "generation modes (--shell batch generation, --center-sector local-neighborhood generation, "
               "and a no-argument random start) and how they build on sectorGen.py/galaxyGeometry.py.",
        prefix_chars='-+')
...
def main():
    ...
    Parses command-line arguments, then dispatches to `run_shell_batch`,
    `run_local_neighborhood`, or (with neither `--shell` nor
    `--center-sector` given) `run_random_start`, depending on which mode
    was selected.
    """
