...
class StarSystem:
    ...
    def estimate_num_objects(self, star):
        ...
        max_objects = self._estimate_max_objects_from_disk_physics(star)
    ...
    def _estimate_max_objects_from_disk_physics(self, star):
        ...
        `star`'s own protoplanetary disk, instead of an arbitrary curve
        fit to stellar mass.
        ...
        Args:
            star (Star): The star whose disk this estimate is derived
                from -- `self.star` for a single star or a P-type binary,
                or individually `self.primary_star`/`self.secondary_star`
                for an S-type binary (see `estimate_num_objects`, this
                method's only caller).

        Returns:
            int: The physically-derived ceiling on this star's object
    ...
    def _mutual_min_distance_au(self, planet, last_planet):
        ...
        `M_star` is `planet.star.mass` rather than `self.star.mass` --
        for a single star or a P-type (close) binary's merged proxy both
        planets' own `.star` already *is* `self.star`, so this is a no-op
        change there, but for an S-type (wide) binary's secondary list
        `self.star` is still the *primary* (see `StarSystem.__init__`'s
        "an S-type pair never merges" branch) -- using it here would
        derive the secondary's own planet spacing against the wrong
        star's mass entirely.
