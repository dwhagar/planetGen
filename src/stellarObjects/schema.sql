...
-- addressing). Because SQLite's `ALTER TABLE ADD COLUMN` can't add a
-- multi-column CHECK after the fact, this version's `sectors` table is
-- rewritten wholesale here (with the CHECK included from the start)
-- rather than incrementally ALTERed -- `_db.py`'s `migrate_database`
-- rebuilds every database from this current schema.sql and copies old
-- data across column-by-column, so this only matters for what a *fresh*
-- database's `sectors` table looks like, not for how an existing one is
-- upgraded.
--
