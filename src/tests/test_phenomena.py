...
Covers the six phenomena generated by `phenomenonGen.py`'s separate, rarer
generation mode -- `compactRemnant.BlackHole`/`NeutronStar`,
`nebulaData.Nebula`, `supernovaRemnantData.SupernovaRemnant`,
`roguePlanetData.RoguePlanet`/`InterstellarComet` -- plus the
`StarSystem(compact_remnant=...)` integration point they anchor through.
...
@pytest.mark.parametrize("cls", [BlackHole, NeutronStar, Nebula, SupernovaRemnant, RoguePlanet, InterstellarComet])
...
@pytest.mark.parametrize("cls", [Nebula, SupernovaRemnant, RoguePlanet, InterstellarComet])
...
def test_standalone_phenomenon_paragraph_list_renders_section_header(cls, markdown):
    # These four are always standalone (never anchor a StarSystem), so
