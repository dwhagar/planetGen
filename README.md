...
`generate.py` is the single unified command-line entry point for every generator in this project — one script, with a subcommand per generation scale:
...
Run `python generate.py <command> --help` for that command's own full option list — each subcommand accepts exactly the same options as running that generator's original standalone script directly (`systemGen.py`, `sectorGen.py`, `galaxyGen.py`, `galaxyPlan.py`, `phenomenonGen.py` — still present and still directly runnable, since some of them call into each other internally), and every command saves what it generates to the database. The rest of this section, and [Sector Generation](#sector-generation)/[Galaxy Generation](#galaxy-generation)/[Exotic Phenomena Generation](#exotic-phenomena-generation) below, document each subcommand's own options in detail using its standalone script's name; wherever you see e.g. `python systemGen.py [options]`, `python generate.py system [options]` does the same thing.

To generate a new star system directly:
...
(equivalently, `python generate.py sector ...` — see [Usage](#usage) above.)
...
(equivalently, `python generate.py galaxy ...` — see [Usage](#usage) above.)
...
(equivalently, `python generate.py phenomenon ...` — see [Usage](#usage) above.)
